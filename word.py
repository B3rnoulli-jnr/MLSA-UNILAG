stop_words = set([
    ...
    'wouldn', "wouldn't","isnt"
])

def clean_text(text):
    clean_text = '' # Starts with an empty string
    for char in text:
        if char.isalnum() or char == " ":
       # Keep only letters and space (Remove puctuations)
            clean_text = clean_text + char.lower()
    list_word = clean_text.split()
    return list_word

# Function to remove stop words
def remove_stopwords(list_word):
    filtered_word = []
...
# Function to count the words in the list
def count(filtered_word):
    count_words = {}
...
#list_word = cleaned_text.split() # Split the clean text into individual words
filtered_word = remove_stopwords(cleaned_text)
counted_words = count(filtered_word)
top_10_words = top10_words(counted_words)
# suggest title based on top 10 words
good_words = [word for word, count in top_10_words if word not in stop_words]
suggest_title = ' '.join(good_words).title()
#print(cleaned_text)
#print(filtered_word)
#print(counted_words)
#print(top_10_words)
print("Suggested title: ", suggest_title[:20])
